...
from admins.forms import UserAdminRegisterForm, UserAdminProfileForm, CategoryCreateFormAdmin
...
def admin_category_update(request, pk):
    ...
    if request.method == 'POST':
        form = CategoryCreateFormAdmin(data=request.POST, instance=productcategory)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('admins:admin_category'))
    else:
        form = CategoryCreateFormAdmin(instance=productcategory)
    context = {
        'title': 'Geekshop - Админ | Обновление категории',
        'form': form,
        'user_select': productcategory,
    }
